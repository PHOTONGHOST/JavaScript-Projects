...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            //Add to the previous end x endpoint.
            if (x < x2) { x += 10; }
            //Add to the previous end y endpoint.
            if (y < y2) { y += 10; }
            //Stop when we’ve reached both endpoints.
            if (x >= x2 && y >= y2) { cancelAnimationFrame(animationLoop); }
        }

        //This condition is necessary for the 6, 4, 2 win condition (right→left diagonal).
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) { x += 10; }
            if (y > y2) { y -= 10; }
            if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop); }
    ...
    //Disallow clicking while the win sound is playing.
    disableClick();
    //Play the win sound.
    audio('./media/winGame.mp3');
    //Call our main animation loop.
    animateLineDrawing();
